...
// Single endpoint merging the previous whatsapp-manual-send.js and
// whatsapp-templates.js — required to stay within Vercel Hobby's 12
// Serverless Function limit. Behavior is unchanged; only the URL shape
// changed, via a required ?action= query param:
//
//   ?action=manual-send   GET  -> history list (filters: round_id, status, recipient, date)
//   ?action=manual-send   POST -> send one real Twilio WhatsApp message per recipient
//   ?action=templates     GET  -> list templates + approval status
//   ?action=templates     PATCH?name=xxx -> update a template's approval status
//
// Template gating (manual-send refuses non-Approved templates) and all
// other rules are identical to before.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "manual-send") return await handleManualSend(req, res);
    return res.status(400).json({ success: false, error: "action must be 'manual-send' or 'templates'." });
  } catch (error) {
...
// ---------------------------------------------------------------------
// action=templates  (formerly api/whatsapp-templates.js)
// ---------------------------------------------------------------------
...
// ---------------------------------------------------------------------
// action=manual-send  (formerly api/whatsapp-manual-send.js)
// ---------------------------------------------------------------------
