...
// or start+30 days if no future round exists yet for that department.
export async function computePlanDates(roundPlannedDate, department) {
  ...
  if (candidates.length) {
    ...
    if (due <= startDate) due = addDays(startDate, 21);
    return { startDate, dueDate: due, nextRoundId: candidates[0].id };
  }
  return { startDate, dueDate: addDays(startDate, 30), nextRoundId: null };
}
