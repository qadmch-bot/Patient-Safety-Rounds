...
export async function sendTwilioMessageRaw({ to, message }) {
  ...
  try {
    ...
    const callbackBase = process.env.PUBLIC_APP_URL || process.env.APP_URL || process.env.VERCEL_PROJECT_PRODUCTION_URL;
    if (callbackBase) {
      const base = callbackBase.startsWith("http") ? callbackBase : `https://${callbackBase}`;
      form.append("StatusCallback", `${base.replace(/\/$/, "")}/api/twilio-status`);
    }
